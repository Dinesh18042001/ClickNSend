...
//     await axiosInstance
//       .get(`api/auth/payment/create-account66/${user.id}
//       `)
//       .then((response) => {
//         if (response.status === 200) {
...
const StripePage = () => {
  ...
  const fetchTrackJob = async (user) => {
    try {
      ...
      const response = await axiosInstance.get(`/api/auth/payment/connect/${user.id}`);
      if (response.status === 200) {
        setData(response.data); // Corrected to set response data
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleFetchTrackJobss = async () => {
    await axiosInstance
      ...
      .then((response) => {
        if (response.status === 200) {
          setCreateAccountData(response.data); // Corrected to set response data
        }
      })
      .catch((error) => console.log(error));
  };
  ...
  const handleFetchTrackJobs = async () => {
    await axiosInstance
      ...
      .then((response) => {
        if (response.status === 200) {
          setLinkAccountData(response.data); // Corrected to set response data
        }
      })
      .catch((error) => console.log(error));
  };
  ...
  const handleClick = async () => {
    setAnchorEl(null);
    try {
      if (data && data.details && data.details.charges_enabled) {
        await handleFetchTrackJobs();
        console.log('handleFetchTrackJobs');
        if (linkAccountData && linkAccountData.url) {
          window.location.href = linkAccountData.url; // Redirect to Stripe URL
        }
      } else {
        await handleFetchTrackJobss();
        console.log('handleFetchTrackJobss');
        if (createAccountData && createAccountData.url) {
          window.location.href = createAccountData.url; // Redirect to Stripe URL
        }
      }
    } catch (error) {
      console.log(error);
